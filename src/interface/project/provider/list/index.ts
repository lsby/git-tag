...
let 接口路径 = '/api/project/provider/list' as const
...
let 接口逻辑实现 = 接口逻辑
  ...
  .绑定(
    接口逻辑.构造([new JSON参数解析插件(z.object({}), {}), kysely插件], async (参数, 逻辑附加参数, 请求附加参数) => {
      ...
      return new Right({ providers: 结果.map((row) => ({ id: row.id, providerType: row.provider_type })) })
    }),
  )
...
let 接口正确类型描述 = z.object({ providers: z.array(z.object({ id: z.string(), providerType: z.string() })) })

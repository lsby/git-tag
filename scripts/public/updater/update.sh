...
expand_validated_zip() {
  ...
  local staged_app="$STAGING_DIR/lsby-git-tag.app"
  local staged_pkg="$staged_app/Contents/Resources/app/package.json"
  local staged_bin="$staged_app/Contents/MacOS/lsby-git-tag"
...
invoke_prisma() {
  ...
  local electron_bin="$macos_dir/lsby-git-tag-bin"
  if [ ! -f "$electron_bin" ]; then
    electron_bin="$macos_dir/lsby-git-tag"
  fi
...
STAGED_APP_DIR="$STAGING_DIR/lsby-git-tag.app"
...
if [ -d "$ROOT/lsby-git-tag.app" ]; then
  chmod -R +x "$ROOT/lsby-git-tag.app/Contents/MacOS" 2>/dev/null || true
  xattr -dr com.apple.quarantine "$ROOT/lsby-git-tag.app" 2>/dev/null || true
fi

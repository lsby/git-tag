...
async function 执行构建(): Promise<void> {
  try {
    ...
    if (process.platform === 'win32') {
      ...
      let 构建程序目录名 =
        子项组.find((项) => fs.existsSync(path.join(待清理路径, 项, 'lsby-git-tag.app')) === true) ??
        子项组.find((项) => 项.startsWith('mac'))
      if (构建程序目录名 === undefined) {
        throw new Error('❌ 未找到 electron-builder 生成的 macOS .app 目录')
      }
      let 源App路径 = path.join(待清理路径, 构建程序目录名, 'lsby-git-tag.app')
      生成目录 = path.join(待清理路径, 'mac-portable')
      确保目录存在(生成目录)
      let 目标App路径 = path.join(生成目录, 'lsby-git-tag.app')
    ...
    let appResourcesEnv = path.join(生成目录, 'lsby-git-tag.app/Contents/Resources/app/.env')
    ...
    let appResourcesDb = path.join(生成目录, 'lsby-git-tag.app/Contents/Resources/app/db')
    ...
    let appPrisma目标目录 =
      process.platform === 'win32'
        ? path.join(生成目录, 'app')
        : path.join(生成目录, 'lsby-git-tag.app/Contents/Resources/app')
    ...
    if (process.platform === 'win32') {
      for (let 文件名 of ['lsby-git-tag-debug.cmd', 'update.cmd']) {
    ...
    if (process.platform === 'darwin') {
      let macOsBinDir = path.join(生成目录, 'lsby-git-tag.app/Contents/MacOS')
      let 原生可执行文件 = path.join(macOsBinDir, 'lsby-git-tag')
      let 真实二进制文件 = path.join(macOsBinDir, 'lsby-git-tag-bin')
      ...
      if (fs.existsSync(原生可执行文件) === true) {
        ...
        let appLauncher脚本 = [
          ...
          '  fi',
          'fi',
          'export DEBUG="@lsby:*,@lsby:git-tag:*"',
          'exec "$DIR/lsby-git-tag-bin" "$@"',
        ].join('\n')
    ...
    if (Prisma迁移名称组.length > 0) {
      let PrismaCli路径 =
        process.platform === 'win32'
          ...
          : path.join(生成目录, 'lsby-git-tag.app/Contents/Resources/app/node_modules/prisma/build/index.js')
      ...
      let Prisma环境 = {
        ...
        NODE_PATH:
          process.platform === 'win32'
            ...
            : path.join(生成目录, 'lsby-git-tag.app/Contents/Resources/app/node_modules'),
      }
    ...
    if (process.platform === 'win32') {
      ...
      let runExe路径 = path.join(生成目录, 'lsby-git-tag.exe')

      if (cscPath === null || fs.existsSync(cscPath) === false) {
        console.warn(`⚠️ 未找到 C# 编译器，跳过 lsby-git-tag.exe 的编译。`)
      } else if (fs.existsSync(launcher源文件) === false) {
        console.warn(`⚠️ 未找到引导器源码: ${launcher源文件}，跳过 lsby-git-tag.exe 的编译。`)
      } else {
        console.log('✅ 正在编译引导器 lsby-git-tag.exe ...')
        try {

...
async function 执行构建(): Promise<void> {
  try {
    ...
    let 可执行文件名 = process.platform === 'win32' ? 'lsby-git-tag.exe' : 'lsby-git-tag'
    ...
    if (process.platform === 'win32') {
      ...
      fs.writeFileSync(path.join(发布目录, 'lsby-git-tag-debug.cmd'), 启动脚本内容)
      console.log(`✅ 已生成 ${path.join(发布目录, 'lsby-git-tag-debug.cmd')}`)

      // 生成 start.exe (C# 引导器)
      let cscPath = 寻找内置Csc编译器()
      let launcher源文件 = path.join(__当前目录名, 'launcher', 'launcher.cs')
      let runExe路径 = path.join(发布目录, 'lsby-git-tag.exe')
      ...
      let 启动脚本内容 = [
        ...
        'echo "=================================================="',
        'echo "lsby-git-tag (SEA单文件服务) 启动引导器"',
        'echo "=================================================="',
        'echo "正在启动后台服务..."',
        './lsby-git-tag',
        'EXIT_CODE=$?',
